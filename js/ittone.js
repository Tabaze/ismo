var ittone = (function () {
    ...
    const wooUrlProducts = '/wp-json/wc/v3/products';
    const wooUrlCategories = '/wp-json/wc/v3/products/categories';
    const reportsURL = '/wp-json/wc/v3/reports/products/totals';
    const wooBatch = '/wp-json/wc/v3/products/batch';
    var wooClientKey = '';
    var wooClientSecret = '';
    var domainName = '';
    var auth = '';
    return {
        init: function () {
            ...
            if ($('.ui.calendar.inputTime').length) {
                $('.ui.calendar.inputTime').inputTime();
            }
            ...
            if (pathname != '/Default.aspx') {
                let lt = ittone.AjaxJson('ModeleDossier.aspx/getInfoWoocommerce', JSON.stringify({ Id_dossier: ittone.idDossier() }))[0];
                if (lt) {
                    domainName = lt.domainName
                    wooClientKey = lt.wooClientKey
                    wooClientSecret = lt.wooClientSecret
                }
            }
        ...
        AjaxJsonAsync: function (url, data, beforeSend, complete) {
            $.ajax({
                type: 'POST',
                url: url,
                async: true,
                ...
                complete: function (data) { complete(data.responseJSON.d) },
                error: function (xhr, status, error) {
                    var err = eval("(" + xhr.responseText + ")");
                    //alertify.error(err.Message);
                    ittone.error(err.Message);
        ...
        UploaderImage: function ($input, path, fileName) {
            ...
            if ($input.prop('files').length > 0) {
                ...
                formdata.append("fileName", fileName.split('\\').pop().split("."));
                console.log(path)
                $.ajax({
                    type: 'POST',
            ...
            <div class="modal-dialog modal-md modal-dialog-centered alert">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title">`+ i18n.translate(title) + `</h5>
                ...
                <div class="modal-footer">
                  <button type="button" class="btn btn-light btn-annuler btn-rounded">`+ i18n.translate("Cancel") + `</button>
                  <button type="submit" class="btn btn-success btn-sauvegarder btn-rounded mx-2">`+ i18n.translate("OK") + `</button>
                </div>
              </div>
            </div>  
          </div>    
        ...
        alert2: function (title, msg, oui, non, annuler) {
            let $modalAlert = $(`
          <div role="dialog" class="modal d-none">
            <div class="modal-dialog modal-md modal-dialog-centered alert">
              <div class="modal-content">
                ...
                <div class="modal-footer">
                  <button type="button" class="btn btn-light btn-annuler btn-rounded">`+ i18n.translate("Cancel") + `</button>
                  <button type="button" class="btn btn-danger btn-rounded btn-non">`+ i18n.translate("non") + `</button>
                  <button type="button" class="btn btn-success btn-sauvegarder btn-rounded mx-2">`+ i18n.translate("oui") + `</button>
                </div>
              </div>
            </div>  
          </div>    
            ...
            $modalAlert.on('click', '.btn-non', function () {
                non();
                ittone.hide($modalAlert);
                $modalAlert.remove();
            });
        ...
        lodingShow: function () {
            let div = `<div class="Lodingoverly"><div class="ittoneOverly1" >
      <div class="ittoneOverly2" style="width: 100%; display: grid;">
          <div class="ittoneOverly3" style="display: flex;">
              <div class="shapes-7"></div>
          </div>
          <h4 class="ittoneOverly-title" style="display: block;">Chargement...</h4>
      </div>
  </div></div>`;
            $('body').append(div);
        },
        lodingHide: function () {
            let $loding = $('body').find('.Lodingoverly');
            $loding.hide("drop", { direction: "right" }, "slow");
            setTimeout(() => {
                $loding.remove();
            }, 3000);
        ...
        convertTime: function (time) {
            if (time) {
                let m = moment();
                m.set({ hour: time.Hours, minute: time.Minutes })
                return m.format('LT');
            }
            else
                return null;
        ...
        escapeRegExp: function (string) {
            return string.replace(/[^a-zA-Z ][1-9]/g, "").replace(/\s/g, '');
        },
        ...
        getWooDataAsync: (type, id) => {
            return new Promise((resolve, reject) => {
                let list = [];
                let woourl = domainName + (type == 'categorie' ? wooUrlCategories + '/?per_page=100' : wooUrlProducts);
                if (auth == '') {
                    auth = basicAuth();
                }
                if (id) {
        ...
        postWooDataAsync: (data, type) => {
            return new Promise((resolve, reject) => {
                let woourl = domainName + (type == 'categorie' ? wooUrlCategories : wooUrlProducts)
                if (data.id) {
                    woourl += "/" + data.id
                }
                if (auth == '') {
        ...
        deleteWooData: (id, type) => {
            let woourl = domainName + (type == 'categorie' ? wooUrlCategories : wooUrlProducts) + '/' + id
            if (auth == '') {
                auth = basicAuth();
            }
            $.ajax({
        ...
        defaultImgEmploye: '../../image/app/employe.png',
        pathImgArticle: 'image/data/article/',
        pathImgEmploye: '../../image/data/employes/',
...
const modal = function () {
    ...
    $.modal.on('click', '.btn-close', function () {
        ittone.hide($(this).closest('.modal[role="dialog"]'));
    });
}
...
const forms = function () {
    ...
    $('input[required]').closest('.form-outline,.inputDate input').css('background', 'rgb(255, 255, 97)')
    setTimeout(() => {
        $('select[required],select[required] + .select2,select[required] + .select2 .select2-selection').css('background', 'rgb(255, 255, 97)')
    }, 1000)
...
const setValueForms = function (form, name, value) {
    ...
    //console.log($(queryElements).val())

}
const headerMenuLongText = function () {
    const $autoNav = $("#menu-header");
    const $autoNavWidth = $autoNav.height();
    const $width = $autoNav.outerWidth(true);
    var menu4 = 0
    var menu3 = 0;
    $('#menu-header > li').each(function () {
        menu3 += $(this).outerWidth(true);
    });
    if ($autoNav.find('li.nav-item:not(#autoNavMoreList li.nav-item)').length == 0) {
        return false;
    }
    if ($autoNavWidth > 48 && $width < (menu3 + 10)) {
      ...
      <i class="fas fa-plus-square more-btn fs-5"></i>
        ...
        //console.log($autoNav.find('li.nav-item:not(#autoNavMoreList li.nav-item)').length)
        headerMenuLongText();
    }
}
...
const basicAuth = () => {
    let lt = ittone.AjaxJson('ModeleDossier.aspx/getInfoWoocommerce', JSON.stringify({ Id_dossier: ittone.idDossier() }))[0];
    if (lt) {
        let hash = btoa(lt.wooClientKey + ':' + lt.wooClientSecret);
        return "Basic " + hash;
    }
}
...
jQuery.fn.extend({
    ...
    setTime: function (value, change = false) {
        if (value) {
            let m = moment();
            m.set({ hour: value.Hours, minute: value.Minutes, second: value.Seconds, millisecond: value.Milliseconds });
            $(this).calendar("set date", moment(m).toDate(), true, change);
        }
    },
    getDate: function () {
        return moment($(this).calendar("get date")).add(12,'hours').toDate();
    },
    // getTime: function () {
    //   return moment($(this).calendar("get date")).format('LT');
    // },
    getTime: function () {
        var time = moment($(this).calendar("get date")).format('LT');
        if (time === '') {
            return null;
        } else {
            return time;
    ...
    randerTable: function (columns_title, data, opt, order) {
