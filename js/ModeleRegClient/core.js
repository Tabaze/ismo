...
import {selectClient,templateClientInsert} from '../ModeleTier/component.js';
import {selectEntetByIdClient} from '../ModeleEntet/component.js';
import {clientCredit} from './service.js';
...
$(function () {
    ...
    $('#idClient').on('change',this,function(){
        ...
        if(data){
            ...
            selectEntetByIdClient($('#idEntet'),{id:data.idClient});
           let credit =  clientCredit({id:data.idClient,idDossier:ittone.idDossier()});
           if(credit.length){
...
const loadApp=function(){
    ...
    selectEntetByIdClient($('#idEntet'),{id:-1});
    if(name=='ESPC' || name=='RNDC'){
        $('#numCheque').closest('.form-outline').remove();
    }
    if(name=='CHFFC' ||name=='EFFETCL' ){
